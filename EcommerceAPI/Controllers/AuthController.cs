    ...
    public class AuthController : ControllerBase
    {
        ...
        public async Task<IActionResult> Register([FromBody] RegisterUserDTO model)
        {
            try
            {
                ...
                // Get the role of the currently logged-in user (default to "User" if not found)
                var requestingUserRole = User?.FindFirst(ClaimTypes.Role)?.Value ?? "User";

                var success = await _authService.RegisterUserAsync(model, requestingUserRole);
